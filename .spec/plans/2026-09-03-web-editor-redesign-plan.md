...
约束：不改 0-7 / 0-8；不加运行时依赖；不引 `@univerjs-pro/*`；CSP 下不引外链；Host 只在 M6-K 改；每张 Workflow 卡在仓内按 `writing-plans` 再拆成文件集互斥的任务卡并行扇出；`App.tsx` 不进重设计实现卡（M6-G～M6-J）的 worker 文件集（M6-F 只改 `canEdit` 安装时序），每 wave 合入时由主 loop 接线；每卡只跑覆盖本卡的测试，统一合入后跑全量：
